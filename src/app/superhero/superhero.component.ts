...
export class SuperheroComponent implements OnInit {
  ...
  ngOnInit(): void {
    this.superHeroService.getAll().then((res) => { this.superHeroes = res }).catch((err) => { this.toastSrv.showToastError(err) });
  }
  ...
  save(): void {
    ...
    if (this.editingId > 0) {
      ...
      this.superHeroService.update(sh).then((res) => {
        this.form.reset();
        const index = this.superHeroes.findIndex(hero => hero.id === res.id);
      ...
      this.superHeroService.add(sh).then((res) => {
        this.form.reset();
        this.editDialog = false;
  ...
  delete(id: number) {
    this.confirmationService.confirm({
      ...
      accept: () => {
        this.superHeroService.delete(id).then((res) => {
          this.form.reset();
  ...
  search() {
    ...
    this.superHeroService.getByName(this.searchStr).then((res) => { this.superHeroes = res }).catch((err) => { this.toastSrv.showToastError(err) });
  }
